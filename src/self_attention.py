...
class ScaledDotProductSelfAttentionHead(nn.Module):
    def __init__(
        ...
        self.register_buffer(
            "mask", torch.tril(torch.ones(max_block_size, max_block_size))
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        weights = (
            Q @ K.transpose(-2, -1)
        ) * self.scaling_factor  # has shape (B, block, emb) @ (B, emb, block) -> (B, block, block)
        ...
        output = (
            weights @ V
        )  # shape (B, block, block) @ (B, block, head_size) -> (B, block, head_size)
        return output
...
class MultiHeadAttention(nn.Module):
    def __init__(
        ...
        self.heads = nn.ModuleList(
            [
                ScaledDotProductSelfAttentionHead(
